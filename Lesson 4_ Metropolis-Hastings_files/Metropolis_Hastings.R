...
ybar = mean(y)
n = length(y)
hist(y, freq=FALSE, xlim=c(-7.0, 2.0)) # histogram of the data
...
plot(density(post$mu_keep, adjust=2.0), main="", xlim=c(-7.0, 2.0), xlab=expression(mu)) # plot density estimate of the posterior
# curve(dt(x=x, df=1), lty=2, add=TRUE) # prior for mu
points(ybar, 0, pch=19) # sample mean
curve(0.017*exp(lg(mu=x, n=n, ybar=ybar)), from=-1.0, to=3.0, add=TRUE, col="blue") # approximation to the true posterior in blue

summary(post$mu_keep)